...
export class InstructionsParser {
    parse(machineCode: DataView, codeStart: number, codeLength: number, addrOffset: number): InstructionByAddress {
        const instructions: InstructionByAddress = {};
        let i = codeStart;
        while (i < codeStart + codeLength) {
            const instructionParser = new InstructionParser(machineCode, i, addrOffset);
