...
//0 32 44
export const rawInstructionDefinitions = `
...
export const instructionDefinitionsByOpCode = new Map<
  string,
...
function parseInstructionDefinitions() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const mnemonicInstructionStr: string =
      indexFirstSpace === -1
        ? mnemonicStr
        : mnemonicStr.substring(0, indexFirstSpace);
    const instructionType: InstructionType =
      InstructionType[mnemonicInstructionStr as keyof typeof InstructionType];
    if (instructionType === undefined) {
      throw new Error(
        `Undefined mnemonic: ${mnemonicInstructionStr} on line ${line}.`
      );
    }
    ...
    if (indexFirstSpace !== -1) {
      ...
      for (const operandStr of operandsStr) {
        const operandType: OperandType | undefined =
          operandStrToOperandType[operandStr];
    ...
    if (modRMExtension && opcodeBytes.length !== 1) {
      throw new Error(
        `There can't be a register code with an opcode that's more than 1 byte long! On line ${i}`
      );
    }
    ...
    if (operands.length > 0) {
      if (
        operands[0] === OperandType.regOrMem8 ||
