...
function getSibIndexRegister(
  index: number,
  width: SubRegisterWidth
): Register | null {
...
export class InstructionParser {
  ...
  notImplemented() {
    throw new Error(
      'Instruction not implemented: ' + this.instruction.opCode.toString(16)
    );
  }

  getModRmRegister(modrmreg: ModRMReg, instruction: InstructionRaw): Register {
    const registerType: RegisterType = RegisterType.integer;
    ...
    if (instruction.is8BitsInstruction) {
      ...
      const operandWidth = this.getOperandSize(
        this.instructionDefinition,
        this.instruction.operands.length
      );
      switch (operandWidth) {
  ...
  parseRegBits() {
    ...
    this.instruction.operands.push({
      register: this.getModRmRegister(regEReg, this.instruction),
    });
  }
  ...
  getOperandSize(
    ...
    const allOps: InstructionDefinition[] | undefined =
      instructionDefinitionsByOpCode.get(uniq);
    if (allOps === undefined) {
      throw new Error(
        `Can't find instruction definitions with identifier ${uniq}`
      );
    }
    ...
    switch (maxWidth) {
      ...
      case OperationSize.qword:
        if (this.instruction.operandSizeOverride) {
          ...
          !!this.default64OperandSize[this.instruction.type] ||
          (this.instruction.rex !== undefined && this.instruction.rex.w)
        ) {
  ...
  parseRmBits(instructionDefinition: InstructionDefinition) {
    ...
    const dataSize = this.getOperandSize(
      instructionDefinition,
      this.instruction.operands.length
    );
    ...
    if (this.instruction.modRM.mod === 0x03) {
      ...
      this.instruction.operands.push({
        register: this.getModRmRegister(regERm, this.instruction),
      });
    } else {
      ...
      if (
        this.default64MemoryOffset[this.instruction.type] ||
        (this.instruction.rex && this.instruction.rex.w)
      ) {
        width = SubRegisterWidth.qword;
      ...
      if (
        rmModNot11b === ModRMrmMobNot11b.SIB ||
        rmModNot11b === ModRMrmMobNot11b.SIB2
      ) {
        ...
        const baseReg: Register | null = getSibBaseRegister(
          baseExtended,
          width,
          this.instruction.modRM.mod
        );
        const indexReg: Register | null = getSibIndexRegister(
          indexExtended,
          width
        );
        ...
        const operand: Operand = {
          effectiveAddr: {
            ...
            index: indexReg,
            scaleFactor,
            displacement: this.parseDisplacement(
              this.instruction.modRM.mod,
              this.instruction.sib.base
            ),
            dataSize,
          },
        };
  ...
  parseRelativeOffsetAsOperand(maxSize: OperationSize): void {
    ...
    this.instruction.operands.push({
      relativeOffset: {valueUnsigned, valueSigned, width},
    });
  }
  ...
  parseImmediate1632Or64AsOperand(maxImmediateSize: OperationSize): void {
    ...
    this.instruction.operands.push({
      immediate: {valueUnsigned, valueSigned, width},
    });
  }
  ...
  getRegisterInOpCode1632or64(mask: number): Register {
    const defaultTo64 = !!this.default64OperandSize[this.instruction.type];
    const regValue = this.instruction.opCode ^ mask;
    if (this.instruction.operandSizeOverride) {
      if (this.instruction.rex && this.instruction.rex.b) {
        switch (regValue) {
    ...
    throw new Error(
      `The opcode ${this.instruction.opCode.toString(
        16
      )} doesn't contain a register value!`
    );
  }

  extratROperand1632or64(mask: number) {
    this.instruction.operands.push({
      register: this.getRegisterInOpCode1632or64(mask),
    });
  }
  ...
  parse(): Instruction {
    ...
    this.readRex();
    const instructionBeginI = this.bytei;
      ...
      if (canBeTheInstruction) {
        ...
        switch (id.mnemonic.operands[0]) {
          ...
          case OperandType.RAX:
            {
              const allOps: InstructionDefinition[] | undefined =
                instructionDefinitionsByOpCode.get(id.opCode.uniq);
              if (allOps === undefined) {
                switch (id.mnemonic.operands[0]) {
                  case OperandType.AX:
              ...
              if (maxOperand === OperandType.RAX) {
                if (
                  this.instruction.rex !== undefined &&
                  this.instruction.rex.w
                ) {
                  this.instruction.operands.push({register: Register.RAX});
                } else if (this.instruction.operandSizeOverride) {
                  this.instruction.operands.push({register: Register.AX});
        ...
        if (allOps === undefined) {
          throw new Error(
            `Can't find InstructionDefinition array for identifier ${id.opCode.uniq}.`
          );
        }
        if (id.opCode.immediateSize !== undefined) {
          ...
          for (const op of allOps) {
            if (
              op.opCode.immediateSize !== undefined &&
              op.opCode.immediateSize > maxImmediateSize
            ) {
        ...
        if (id.opCode.codeOffsetSize !== undefined) {
          ...
          for (const op of allOps) {
            if (
              op.opCode.codeOffsetSize !== undefined &&
              op.opCode.codeOffsetSize > maxCodeOffsetSize
            ) {
